...
- `npm test` — twenty-eight Node unit tests, including Gemini provider routing, action-container retry selection, visible destructive-control guards, queue error-callout containment, route/hidden-tab queue safety, locale completeness, and fallback substitutions; latest result: pass.
