(function (global) {
  ...
  function extractKimiConversations(documentRef) {
    const byId = new Map();
    for (const link of documentRef.querySelectorAll('a[href*="/chat/"]')) {
      const id = kimiConversationIdFromHref(link.href, documentRef.location?.origin);
  ...
  class KimiAdapter {
    ...
    list() { return extractKimiConversations(this.document); }
    ...
    async deleteConversation(item) {
      ...
      if (!link) throw new Error('Kimi conversation did not return to the sidebar after it refreshed.');
      ...
      if (!more) throw new Error('Kimi conversation action menu was not found for the selected sidebar item.');
      more.click();
      ...
      throw new Error('Kimi kept the deletion confirmation open and did not update the sidebar.');
    }
    ...
    findActionButton(container) {
      ...
      return buttons.find((button) => {
        ...
        return isVisible(button) && (labelIncludes(button, KIMI_MORE_LABELS) || /(?:more|option|menu|action)/i.test(testId));
      }) || null;
    }
  ...
  global.KimiAdapter = KimiAdapter;
  global.quickdelKimiHelpers = { kimiConversationIdFromHref, extractKimiConversations, isVisible };
  if (typeof module !== 'undefined') module.exports = { KimiAdapter, kimiConversationIdFromHref, extractKimiConversations, isVisible };
})(globalThis);