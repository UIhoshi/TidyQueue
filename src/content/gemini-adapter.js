(function (global) {
  const GEMINI_DELETE_LABELS = ['delete', '删除', 'eliminar', 'supprimer', 'löschen', '削除', '삭제', 'excluir', 'elimina'];
  const GEMINI_MORE_LABELS = ['more', 'option', '更多', 'más', 'plus', 'mehr', 'その他', '더보기', 'mais', 'altro'];
  ...
  function geminiConversationIdFromHref(href, base = global.location?.origin || 'https://gemini.google.com') {
    try {
  ...
  function extractGeminiConversations(documentRef) {
    const byId = new Map();
    for (const link of documentRef.querySelectorAll('a[href*="/app/"]')) {
      const id = geminiConversationIdFromHref(link.href, documentRef.location?.origin);
  ...
  class GeminiAdapter {
    ...
    list() { return extractGeminiConversations(this.document); }
    ...
    async deleteConversation(item) {
      ...
      if (!deleteAction) throw new Error('Gemini delete action was not found in the selected menu.');
      deleteAction.click();

      const confirm = await waitFor(() => this.findVisibleConfirmation(), 2500);
      if (!confirm) throw new Error('Gemini delete confirmation dialog was not found.');
      confirm.click();
      ...
      throw new Error('Gemini kept the deletion confirmation open and did not update the sidebar.');
    }
    ...
    findActionButton(container) {
      ...
      return buttons.find((button) => {
        ...
        return isVisible(button) && (labelIncludes(button, GEMINI_MORE_LABELS) || /(?:more|option|menu|action)/i.test(testId));
      }) || null;
    }
    ...
    findVisibleMenuAction() {
      ...
      return candidates.find((node) => isVisible(node) && labelIncludes(node, GEMINI_DELETE_LABELS)) || null;
    }

    findVisibleConfirmation() {
      const dialogs = [...this.document.querySelectorAll('[role="dialog"], [role="alertdialog"]')].filter(isVisible);
      for (const dialog of dialogs) {
        const button = [...dialog.querySelectorAll('button, [role="button"]')].find((node) => isVisible(node) && labelIncludes(node, GEMINI_DELETE_LABELS));
  ...
  global.quickdelGeminiHelpers = { geminiConversationIdFromHref, extractGeminiConversations, isVisible };
  if (typeof module !== 'undefined') module.exports = { GeminiAdapter, geminiConversationIdFromHref, extractGeminiConversations, isVisible };
})(globalThis);
