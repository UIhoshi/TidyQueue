(function (global) {
  ...
  function extractCopilotConversations(documentRef) {
    const byId = new Map();
    for (const link of documentRef.querySelectorAll('a[href*="/chats/"]')) {
      const id = copilotConversationIdFromHref(link.href, documentRef.location?.origin);
  ...
  class CopilotAdapter {
    ...
    findVisibleMenuAction() {
      ...
      return candidates.find((node) => isVisible(node) && labelIncludes(node, COPILOT_DELETE_LABELS)) || null;
    }
  ...
  global.quickdelCopilotHelpers = { copilotConversationIdFromHref, extractCopilotConversations, isVisible };
  if (typeof module !== 'undefined') module.exports = { CopilotAdapter, copilotConversationIdFromHref, extractCopilotConversations, isVisible };
})(globalThis);