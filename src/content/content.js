(function () {
  ...
  function createShell() {
    ...
    shadow.innerHTML = `<style>${styles()}</style><div id="quickdel-layer" hidden></div><button id="qd-fab" type="button" aria-label="Open Quickdel"><span aria-hidden="true">◇</span><b>Quickdel</b><i aria-hidden="true">⌃</i></button>`;
    ...
    layer.innerHTML = `
      <section class="qd-shell qd-theme-${resolvedTheme()}" role="dialog" aria-modal="true" aria-labelledby="qd-title">
        <header class="qd-header">
          <div><p class="qd-eyebrow">QUICKDEL · LOCAL ONLY</p><h1 id="qd-title">${escapeHtml(t('title'))}</h1></div>
          <button class="qd-icon" data-action="close" aria-label="${escapeHtml(t('close'))}">×</button>
        </header>
        <div class="qd-toolbar">
          <div class="qd-segment" role="group" aria-label="View mode">
          ...
          </div>${state.mode === 'list' ? `<div class="qd-density" role="group" aria-label="List density"><span>1–4</span>${[1, 2, 3, 4].map((density) => `<button data-action="density" data-density="${density}" aria-pressed="${state.density === density}">${density}</button>`).join('')}</div>` : ''}
          ...
          <select data-input="age" aria-label="Conversation age filter">
          ...
          <span class="qd-count">${selectionCount} ${escapeHtml(t('selected'))}</span>
        </div>
  ...
  function cardMarkup(item) {
    ...
    return `<article class="qd-card ${selected ? 'is-selected' : ''}" data-card-id="${escapeHtml(item.id)}" role="checkbox" aria-checked="${selected}" tabindex="0">
      <label><input type="checkbox" data-id="${escapeHtml(item.id)}" ${selected ? 'checked' : ''} aria-label="Select ${escapeHtml(item.title)}"><span class="qd-check">✓</span></label>
  ...
  function showReview(reviewScrollTop = 0) {
    ...
    find('progress').setAttribute('aria-label', `0 of ${total} conversations completed`);
  ...
  function showQueue(snapshot) {
    ...
    find('progress').setAttribute('aria-label', `${snapshot.completed} of ${snapshot.total} conversations completed`);
