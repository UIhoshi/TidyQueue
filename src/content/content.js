(function () {
  ...
  const QUEUE_ROW_OPACITY = [1, .75, .5];
  const QUEUE_ROW_HEIGHT = 44;

  function queueSlotMarkup() {
    return [0, 1, 2].map((index) => `<li class="qd-upcoming qd-queue-placeholder" data-queue-slot="${index}" data-queue-depth="${index}" aria-hidden="true"><b></b><span></span></li>`).join('');
  }
  ...
  function ensureQueueModal() {
    ...
    modal.innerHTML = `<div class="qd-backdrop"><section class="qd-modal qd-progress" data-queue-mode="live" role="dialog" aria-modal="true" aria-live="polite" aria-labelledby="qd-progress-title">
      ...
      <p class="qd-notice qd-notice-empty" data-queue="notice" aria-hidden="true"> </p><h3 data-queue="up-next"></h3><ol class="qd-live-list" data-queue="list">${queueSlotMarkup()}</ol>
  ...
  function patchQueueRow(row, item, index, sequenceStart) {
    row.dataset.queueSlot = String(index);
    row.dataset.queueDepth = String(index);
  ...
  function isOneRowAdvance(rows, upcoming) {
    const previousIds = rows.map((row) => row.dataset.itemId || '');
    const nextIds = upcoming.map((item) => item?.id || '');
    return Boolean(previousIds[0] && nextIds[0] && previousIds[1] === nextIds[0] && previousIds[2] === nextIds[1]);
  }

  function cancelQueueAnimations(list) {
    list.querySelectorAll('li').forEach((row) => row.getAnimations?.().forEach((animation) => animation.cancel()));
    list.querySelectorAll('.qd-queue-leaving').forEach((row) => row.remove());
  }

  function animateQueueAdvance(list, rows, upcoming, sequenceStart) {
    const [leaving, ...survivors] = rows;
    const leavingGhost = leaving.cloneNode(true);
    leavingGhost.classList.add('qd-queue-leaving');
    leavingGhost.setAttribute('aria-hidden', 'true');
    leavingGhost.style.opacity = String(QUEUE_ROW_OPACITY[0]);
    list.appendChild(leaving);
    list.appendChild(leavingGhost);

    const rotatedRows = [...survivors, leaving];
    rotatedRows.forEach((row, index) => patchQueueRow(row, upcoming[index], index, sequenceStart));

    const motion = { duration: 360, easing: 'cubic-bezier(.22,1,.36,1)' };
    const departure = leavingGhost.animate?.([
      { transform: 'translateY(0)', opacity: QUEUE_ROW_OPACITY[0] },
      { transform: `translateY(-${QUEUE_ROW_HEIGHT}px)`, opacity: 0 }
    ], { ...motion, fill: 'forwards' });
    departure?.finished?.catch(() => {}).then(() => leavingGhost.remove());

    survivors.forEach((row, index) => {
      if (!upcoming[index]) return;
      row.animate?.([
        { transform: `translateY(${QUEUE_ROW_HEIGHT}px)`, opacity: QUEUE_ROW_OPACITY[index + 1] },
        { transform: 'translateY(0)', opacity: QUEUE_ROW_OPACITY[index] }
      ], motion);
    });
    if (upcoming[2]) {
      leaving.animate?.([
        { transform: `translateY(${QUEUE_ROW_HEIGHT}px)`, opacity: 0 },
        { transform: 'translateY(0)', opacity: QUEUE_ROW_OPACITY[2] }
      ], motion);
    }
  }

  function updateQueueRows(panel, upcoming, sequenceStart) {
    const list = queueField(panel, 'list');
    const rows = Array.from(list.querySelectorAll('.qd-upcoming:not(.qd-queue-leaving)'));
    cancelQueueAnimations(list);
    if (rows.length !== 3 || !isOneRowAdvance(rows, upcoming) || prefersReducedMotion()) {
      rows.forEach((row, index) => patchQueueRow(row, upcoming[index], index, sequenceStart));
      return;
    }
    animateQueueAdvance(list, rows, upcoming, sequenceStart);
  }
  ...
  function showQueue(snapshot) {
    ...
    updateQueueRows(panel, snapshot.upcoming, sequenceStart);
