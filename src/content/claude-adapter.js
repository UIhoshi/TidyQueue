(function (global) {
  const CLAUDE_DELETE_LABELS = ['delete', '删除', 'eliminar', 'supprimer', 'löschen', '削除', '삭제', 'excluir', 'elimina'];
  const CLAUDE_MORE_LABELS = ['more', 'option', '更多', 'más', 'plus', 'mehr', 'その他', '더보기', 'mais', 'altro'];
  ...
  function claudeConversationIdFromHref(href, base = global.location?.origin || 'https://claude.ai') {
    try {
  ...
  function extractClaudeConversations(documentRef) {
    const byId = new Map();
    for (const link of documentRef.querySelectorAll('a[href*="/chat/"]')) {
      const id = claudeConversationIdFromHref(link.href, documentRef.location?.origin);
  ...
  class ClaudeAdapter {
    ...
    list() { return extractClaudeConversations(this.document); }
    ...
    async deleteConversation(item) {
      ...
      if (!link) throw new Error('Claude conversation did not return to the sidebar after it refreshed.');
      ...
      if (!more) throw new Error('Claude conversation action menu was not found for the selected sidebar item.');
      more.click();

      const deleteAction = await waitFor(() => this.findVisibleMenuAction(), 2500);
      if (!deleteAction) throw new Error('Claude delete action was not found in the selected menu.');
      deleteAction.click();

      const confirm = await waitFor(() => this.findVisibleConfirmation(), 2500);
      if (!confirm) throw new Error('Claude delete confirmation dialog was not found.');
      confirm.click();
      ...
      throw new Error('Claude kept the deletion confirmation open and did not update the sidebar.');
    }
    ...
    findActionButton(container) {
      ...
      return buttons.find((button) => {
        ...
        return isVisible(button) && (labelIncludes(button, CLAUDE_MORE_LABELS) || /(?:more|option|menu|action)/i.test(testId));
      }) || null;
    }
    ...
    findVisibleMenuAction() {
      ...
      return candidates.find((node) => isVisible(node) && labelIncludes(node, CLAUDE_DELETE_LABELS)) || null;
    }

    findVisibleConfirmation() {
      const dialogs = [...this.document.querySelectorAll('[role="dialog"], [role="alertdialog"]')].filter(isVisible);
      for (const dialog of dialogs) {
        const button = [...dialog.querySelectorAll('button, [role="button"]')].find((node) => isVisible(node) && labelIncludes(node, CLAUDE_DELETE_LABELS));
  ...
  global.quickdelClaudeHelpers = { claudeConversationIdFromHref, extractClaudeConversations, isVisible };
  if (typeof module !== 'undefined') module.exports = { ClaudeAdapter, claudeConversationIdFromHref, extractClaudeConversations, isVisible };
})(globalThis);
