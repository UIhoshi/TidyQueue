...
- `npm test` uses Node's test runner for queue transitions, route/hidden-tab safety guards, ChatGPT/Gemini provider routing and link helpers, Gemini action-container retry selection, queue error-notice containment, card text containment, sidebar-load notice coverage, locale-catalog completeness, and fallback substitutions.
