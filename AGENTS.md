...
- Product: a Chrome Manifest V3 extension that helps a user review and delete their own ChatGPT or Gemini conversations from a visible tab.
...
- Validation: run `npm test` and `npm run package:check` before completion. Browser verification against a logged-in ChatGPT session remains a separate manual step.
